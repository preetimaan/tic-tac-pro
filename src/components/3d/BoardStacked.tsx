...
import { StackedPiece as StackedPieceType } from '../../types/game'
...
export default function BoardStacked() {
  ...
  const board = state.board as StackedPieceType[][]
  const selectedPieceSize = state.selectedPieceSize
  ...
  return (
    <group>
      ...
      {cells.map(({ index, position }) => {
        ...
        // Check if we can show preview for this cell
        const canShowPreview = hoveredCell === index && 
          state.status === 'playing' && 
          selectedPieceSize &&
          state.remainingPieces &&
          state.remainingPieces[currentPlayer][selectedPieceSize] > 0 &&
          canPlacePiece(board, index, selectedPieceSize)
          ...
          <group key={index}>
            ...
            {canShowPreview && (
              <StackedPiece
                ...
                position={position}
                isPreview={true}
                previewSize={selectedPieceSize}
                previewPlayerId={currentPlayer}
              />
            )}
          </group>
        )
